...
static int mipi_himax_lcd_on(struct platform_device *pdev)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (system_rev == 1) {
			mipi_dsi_cmds_tx(&himax_tx_buf, himax_cmd_on_rotation_cmds,
					ARRAY_SIZE(himax_cmd_on_rotation_cmds));
		}
		else {
...
static void mipi_himax_display_off(struct msm_fb_data_type *mfd)
{
...
static struct msm_fb_panel_data himax_panel_data = {
	...
	.display_off = mipi_himax_display_off,
};
