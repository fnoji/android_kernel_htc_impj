...
static int mipi_orise_lcd_on(struct platform_device *pdev)
{
	...
	if (mfd->init_mipi_lcd == 0) {
		...
		mipi_dsi_cmds_tx(&orise_tx_buf, orise_color_enhance,
...
static void mipi_orise_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (led_pwm1[1] == 0) {
		...
		cmdreq_orise.cmds = orise_dim_off_cmds;
		cmdreq_orise.cmds_cnt = ARRAY_SIZE(orise_dim_off_cmds);
...
static void mipi_orise_display_off(struct msm_fb_data_type *mfd)
{
	if (!is_perf_lock_active(&orise_perf_lock))
		perf_lock(&orise_perf_lock);
...
static void mipi_orise_dim_on(struct msm_fb_data_type *mfd)
{
	...
	if (!is_perf_lock_active(&orise_perf_lock))
		perf_lock(&orise_perf_lock);
	cmdreq_orise.cmds = orise_dim_on_cmds;
	cmdreq_orise.cmds_cnt = ARRAY_SIZE(orise_dim_on_cmds);
	cmdreq_orise.flags = CMD_REQ_COMMIT;
	cmdreq_orise.rlen = 0;
	cmdreq_orise.cb = NULL;
	mipi_dsi_cmdlist_put(&cmdreq_orise);
...
static struct msm_fb_panel_data orise_panel_data = {
	...
	.display_off = mipi_orise_display_off,
#ifdef ORISE_CABC
